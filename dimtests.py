...
def data_size_test(algorithm, sets):
    ...
    for data, labels in sets:
        ...
        # print("median runtime: {}".format(avg_runtime))

    print("average runtimes: {}".format(avg_runtimes))
    print("NMI: {}".format(nmi))

    return (dim_sizes, avg_runtimes, nmi)
...
if __name__ == '__main__':
    ...
    dim_sizes = np.logspace(2, 5, 8, dtype=np.int)
    sets = []
    for d in dim_sizes:
    ...
    for alg in algorithms:
        ...
        dim_sizes, avg_runtimes, nmi = data_size_test(alg, sets)
        results[alg.__name__] = (dim_sizes, avg_runtimes, nmi)
        ...
        with open(filename, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(['dim_size', 'avg_runtime', 'StDev', 'NMI'])
            for size, runtime, nmi in zip(dim_sizes, avg_runtimes, nmi):
